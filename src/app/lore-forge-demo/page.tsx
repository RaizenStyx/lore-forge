...
export default function LoreForgeDemo() {
  ...
  // Memoize the filtered lore entries to only re-calculate when type or genre changes
  const filteredTestLore = useMemo(() => {
    return testLoreEntries.filter(
      (entry) => entry.type === type && entry.genre === genre
    );
  }, [type, genre]);

  // Map the filtered entries to a list of prompt strings for the PromptSelector
  const filteredPrompts = filteredTestLore.map(entry => entry.text);

  // Handle the click on a prompt selector button
  const handlePromptSelect = (selectedPromptText: string) => {
    // Find the full lore entry object that matches the selected prompt text
    const selectedLore = filteredTestLore.find(
      (entry) => entry.text === selectedPromptText
    );
    
    if (selectedLore) {
      setPrompt(selectedLore.text);
      setResult(selectedLore.content);
      setSubmittedPrompt(selectedLore.text);
      setTitle(selectedLore.title);
    }
  };
    ...
    <Layout>
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-3 gap-8 p-6 lg:p-8">
        ...
        <section className="lg:col-span-1 space-y-6 p-6 bg-gray-900 rounded-xl shadow-xl ring-1 ring-cyan-500/20 border border-gray-700">
          ...
          <PromptSelector prompts={filteredPrompts} onPromptSelect={handlePromptSelect} />
        </section>
        ...
        <section className="lg:col-span-2 p-6 bg-gray-900 rounded-xl shadow-xl ring-1 ring-cyan-500/20 border border-gray-700 flex flex-col">
          {result ? (
            ...
            <div className="flex-grow flex flex-col items-center justify-center text-gray-400 text-center p-4 text-xl tracking-wide">
              ...
              <p>Select a pre-generated lore prompt from the left panel or below to see how The Lore Forge works!</p>
              <Link href="/lore-forge" className="mt-6 px-6 py-3 bg-cyan-400 hover:bg-fuchsia-500 text-gray-900 font-semibold rounded-full shadow-lg transform hover:scale-105 transition-all duration-300">
                Go to the Starter Lore Forge
              </Link>
              <p className="text-lg mt-2">(This is a demo page to only show what the full Forge will look like.)</p>
            </div>
          )}
        </section>
        ...
        <section className="lg:col-span-3 space-y-6 p-6 bg-gray-900 rounded-xl shadow-xl ring-1 ring-cyan-500/20 border border-gray-700 mt-8">
          <h2 className="text-3xl font-bold text-cyan-400 mb-6 flex items-center gap-3 justify-center">
            Complete Lore Demo Collection
          </h2>

          <p className="text-gray-400 text-center mb-6">
            Below is a full list of all the pre-generated lore examples available in the Test Forge. Click on any entry to load its details and generated content.
          </p>
